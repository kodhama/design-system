...
1. **Load the rules.** Read this file (design-system's self-improvement/
...
3. **Evaluate against the parked-item graph, for real.** design-system
   has no dedicated parked-item store (no TODO/ROADMAP file) — its open
   items ride each artifact's own `## Open questions` section and PR
   bodies. Does this diff fire or action any of those, a trigger
   recorded in a `decisions/` entry, or a feedback artifact's
   disposition? The evidence rule applies: name the exact item and
   *why* the diff touches it — or conclude an honest **"None."** Never
   invent propagation to look thorough.
