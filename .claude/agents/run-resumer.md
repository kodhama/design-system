...
[`charters/run-resumer.md`](https://github.com/kodhama/grove/blob/main/charters/run-resumer.md)).
A dispatched agent run died at its turn cap mid-task. Turn caps are
always reachable — the model is not "avoid the cap," it is
**checkpoint-and-resume**: any run's death must leave enough state that
a successor continues instead of restarting. You are that successor.
...
2. **Find the checkpoint.** Look for a pushed WIP branch and checkpoint
   comments. design-system has no uniform issue-number-in-branch
   convention recorded yet — search `git branch -r | grep <the task's
   own branch slug>` (e.g. `chore/install-grove`-style `<category>/
   <slug>` names, per recent history). **Resume, never redo:** if a
   branch exists, fetch it, verify its state, and continue on it. If
   nothing was pushed, you start clean — say so.
3. **Work the remainder** per the original brief and design-system's own
   discipline (conventional commits, PR-first — agents never merge).
   design-system has no CI-enforced PR-contract section requirement as
   of this writing (flagged, not silently assumed); if one is later
   added (e.g. a required `## Propagation` section), honor it.
...
- **Never weaken the brief to finish faster** (drop tests, skip a spec
  ...
  failure.
- The original brief's own hard constraints bind you (e.g. the
  consumer-contract rule that consuming repos read design-system at a
  pinned git tag, never `main` — check design-system's `.trellis/` and
  CLAUDE.md for its specific constraints).
